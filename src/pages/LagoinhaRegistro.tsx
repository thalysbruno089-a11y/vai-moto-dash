...
import { useLagoinhaDeliveries } from "@/hooks/useLagoinhaDeliveries";
import { format } from "date-fns";
import { Card, CardContent } from "@/components/ui/card";
import { useMemo } from "react";

const LagoinhaRegistro = () => {
  const { signOut } = useAuth();
  const navigate = useNavigate();
  const today = format(new Date(), "yyyy-MM-dd");
  const { data: deliveries = [] } = useLagoinhaDeliveries(today, { sentOnly: false });
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-10 bg-card border-b print:hidden">
        <div className="mx-auto max-w-3xl px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold leading-none">PANIFICADORA LAGOINHA</h1>
      ...
      <main className="mx-auto max-w-3xl px-4 py-4">
        ...
        <LagoinhaDeliveriesBoard editable />
      </main>
    </div>
  );
};

export default LagoinhaRegistro;