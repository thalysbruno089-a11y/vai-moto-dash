...
import {
  useBurgazzoDeliveries,
  useCreateBurgazzoDelivery,
  useUpdateBurgazzoDelivery,
  useDeleteBurgazzoDelivery,
  useSendBurgazzoDayToCentral,
  useBurgazzoDeletionLogs,
  type BurgazzoDelivery,
} from "@/hooks/useBurgazzoDeliveries";
...
interface Props {
  /** Editing (usuário Burgazzo). If false: read-only report view for Carlos/Secretaria. */
  editable?: boolean;
...
function DeliveryRow({
  ...
  delivery: BurgazzoDelivery;
  editable: boolean;
  onPatch: (id: string, patch: Partial<BurgazzoDelivery>) => void;
...
export const BurgazzoDeliveriesBoard = ({
  editable = true,
  ...
  const { data: deliveries = [], isLoading } = useBurgazzoDeliveries(selectedDate, { sentOnly });
  const { data: motoboys = [] } = useMotoboys();
  const createMut = useCreateBurgazzoDelivery();
  const updateMut = useUpdateBurgazzoDelivery();
  const deleteMut = useDeleteBurgazzoDelivery();
  const sendMut = useSendBurgazzoDayToCentral();
  const [toDelete, setToDelete] = useState<string | null>(null);
  const [deletePwd, setDeletePwd] = useState("");
  const [logsOpen, setLogsOpen] = useState(false);
  const { data: deletionLogs = [], isLoading: logsLoading } = useBurgazzoDeletionLogs();
  ...
  const patch = (id: string, p: Partial<BurgazzoDelivery>) => updateMut.mutate({ id, patch: p });
  ...
  return (
    <div className="space-y-4 print:space-y-2">
      ...
      <div className="hidden print:flex print:flex-col print:items-center print:gap-2 print:mb-3">
        <img src={logo} alt="Burgazzo" className="h-16 w-auto" />
        <h2 className="text-xl font-bold">
          Relatório Burgazzo — {format(new Date(selectedDate + "T12:00:00"), "dd/MM/yyyy")}
        </h2>
      </div>
