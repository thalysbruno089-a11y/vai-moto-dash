...
import {
  useLagoinhaDeliveries,
  useCreateLagoinhaDelivery,
  useUpdateLagoinhaDelivery,
  useDeleteLagoinhaDelivery,
  useSendLagoinhaDayToCentral,
  useLagoinhaDeletionLogs,
  type LagoinhaDelivery,
} from "@/hooks/useLagoinhaDeliveries";
...
interface Props {
  /** Editing (usuário Panificadora Lagoinha). If false: read-only report view for Carlos/Secretaria. */
  editable?: boolean;
...
function DeliveryRow({
  ...
  delivery: LagoinhaDelivery;
  editable: boolean;
  onPatch: (id: string, patch: Partial<LagoinhaDelivery>) => void;
...
export const LagoinhaDeliveriesBoard = ({
  editable = true,
  ...
  const { data: deliveries = [], isLoading } = useLagoinhaDeliveries(selectedDate, { sentOnly });
  const { data: motoboys = [] } = useMotoboys();
  const createMut = useCreateLagoinhaDelivery();
  const updateMut = useUpdateLagoinhaDelivery();
  const deleteMut = useDeleteLagoinhaDelivery();
  const sendMut = useSendLagoinhaDayToCentral();
  const [toDelete, setToDelete] = useState<string | null>(null);
  const [deletePwd, setDeletePwd] = useState("");
  const [logsOpen, setLogsOpen] = useState(false);
  const { data: deletionLogs = [], isLoading: logsLoading } = useLagoinhaDeletionLogs();
  ...
  const patch = (id: string, p: Partial<LagoinhaDelivery>) => updateMut.mutate({ id, patch: p });
  ...
  return (
    <div className="space-y-4 print:space-y-2">
      ...
      <div className="hidden print:flex print:flex-col print:items-center print:gap-2 print:mb-3">
        <img src={logo} alt="Panificadora Lagoinha" className="h-16 w-auto" />
        <h2 className="text-xl font-bold">
          Relatório Panificadora Lagoinha — {format(new Date(selectedDate + "T12:00:00"), "dd/MM/yyyy")}
        </h2>
      </div>
